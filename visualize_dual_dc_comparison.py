...
def load_dc_fold_bundle(save_dir: str, split: str, fold: int) -> Dict[str, List[torch.Tensor]]:
    """Load per-fold DC trajectory bundle."""
    path = os.path.join(save_dir, f"{split}_trajs_dc_fold_{fold}.pt")
    if not os.path.isfile(path):
        raise FileNotFoundError(f"Missing DC fold tensor: {path}")
    dc_bundle = torch.load(path)
