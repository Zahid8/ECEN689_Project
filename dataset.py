...
def batch_process_coords(
    ...
    trajs = trajs.to(cfg["device"], non_blocking=True)
    ...
    return (
        ...
        padding_mask.float().to(cfg["device"], non_blocking=True),
    )


def corrupt(cfg, hist_trajs, hist_masks):
    B, hist_len, N, _ = hist_trajs.shape
    device = hist_trajs.device

    corrupt_ratio = random.uniform(cfg.aug.corrupt.ratio.min, cfg.aug.corrupt.ratio.max)
    corrupt_shuffle_indices = torch.rand((B, hist_len * N), device=device).argsort(dim=1)
